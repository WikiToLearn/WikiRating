...
public class Contribution {
	...
	for(Vertex pageNode:graph.getVertices("@class","Page")){
		...
		try{
		pageEdits=0;
		revisionNode=pageNode.getEdges(Direction.OUT,"@class","Pversion").iterator().next().getVertex(Direction.IN);	//Getting the latest version to iterate on the revisons
		...
		while(loopCounter){
			if(revisionNode.getEdges(Direction.OUT,"@class","Version").iterator().hasNext()){
				nextRevisionNode=revisionNode.getEdges(Direction.OUT,"@class","Version").iterator().next().getVertex(Direction.IN);
				pageEdits+=Math.abs((int)nextRevisionNode.getProperty("size")-(int)revisionNode.getProperty("size"));
				revisionNode=revisionNode.getEdges(Direction.OUT,"@class","Version").iterator().next().getVertex(Direction.IN);
			}
			else{
    ...
    while (it.hasNext()) {
        ...
        System.out.println(graph.getVertices("pid",pair.getKey()).iterator().next().getProperty("name")+" has got insertions= "+pair.getValue());
    } 
