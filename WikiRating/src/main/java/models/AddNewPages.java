...
public class AddNewPages {
	static Class className=AddNewPages.class;
	/**
	...
	public static void insertNewPage(OrientGraph graph,JSONObject currentJsonObject,int ns){
		...
		try{
			Loggings.getLogs(className).info("++Adding this new Page++  "+currentJsonObject.getString("title"));
			...
			for (Vertex v : graph.getVertices(key,value)) {
				...
				//JSON interpretation of the fetched String
				 ...
				 try {
					 	...
					 	JSONObject currentJsonObject;
					 	inLinks=arr.length();
					 	Loggings.getLogs(className).info(v.getProperty("title").toString()+" has inLinks = "+inLinks);
					 	...
					 	for(int i=0;i<arr.length();i++){
					 		...
					 		try{
					 			...
					 			Loggings.getLogs(className).info(v.getProperty("title").toString()+" is linked to "+backLink.getProperty("title").toString());
					 	...
					 	for(int i=0;i<arr.length();i++){
					 		...
					 		if(WikiUtil.rCheck("revid", currentJsonObject.getInt("revid"), graph)){
					 			...
					 			if(update==true&&flag){
					 				...
					 				Loggings.getLogs(className).info(pageNode.getProperty("title").toString()+" is getting updated");
					 			...
					 			Loggings.getLogs(className).info("Adding some new revisions");
