...
public class LinkPages {
	static Class className=LinkPages.class;
		...
		for (Vertex pageNode : graph.getVertices(key,value)) {
			...
			//JSON interpretation of the fetched String
			 ...
			 try {
				 	...
				 	Loggings.getLogs(className).info(pageNode.getProperty("title").toString()+" has inLinks = "+inLinks);
				 	...
				 	for(int i=0;i<arr.length();i++){
				 		...
				 		try{
				 			...
				 			Loggings.getLogs(className).info(pageNode.getProperty("title").toString()+" is linked to "+backLink.getProperty("title").toString());
