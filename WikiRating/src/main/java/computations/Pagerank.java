...
public class Pagerank {
	static Class className=Pagerank.class;
	...
	for (Vertex pageNode :graph.getVertices("@class","Page")) {
		...
		Loggings.getLogs(className).info("======= "+pageNode.getProperty("name").toString()+"========"+backLinks);
		for(Edge e:oPageNode.getEdges(Direction.IN,"@class","Backlink")){
			Loggings.getLogs(className).info(e.getVertex(Direction.OUT).getProperty("title"));
		}
	...
	public static void pageRankCompute(){
		...
		//Scaling the values by dividing with the maxlinks
		 Iterator it1 = pageRankMap.entrySet().iterator();
		    ...
		    Loggings.getLogs(className).info(maxLink+" is the max ");
		    ...
		    while (it2.hasNext()) {
		        ...
		        Loggings.getLogs(className).info("===========Finals===========");
		        Loggings.getLogs(className).info(pair.getKey()+"  "+pair.getValue());
		        if(maxPageRank<=(double)pair.getValue()){
		        	maxPageRank=(double)pair.getValue();
		        }
		       }
		    Loggings.getLogs(className).info("=======This is maxPageRank=======  "+maxPageRank);
